    ...
    ($mac:ident) => {
        $mac! {
            @mvp {
                ...
                Forall { blockty: $crate::BlockType } => visit_forall (arity block -> ~block)
                Exists { blockty: $crate::BlockType } => visit_exists (arity block -> ~block)
                Assume { blockty: $crate::BlockType } => visit_assume (arity block -> ~block)
                Unique { blockty: $crate::BlockType } => visit_unique (arity block -> ~block)
                ...
                I32Uzumaki { value: i32 } => visit_i32_uzumaki (push i32)
                I32Const { value: i32 } => visit_i32_const (push i32)
                I64Uzumaki { value: i64 } => visit_i64_uzumaki (push i64)
